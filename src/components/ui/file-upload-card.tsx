      ...
      >
        <div className="p-6 md:p-8">
          <div className="flex items-start justify-between mb-6">
            ...
            {onClose && (
               <Button variant="ghost" size="icon" className="rounded-full w-10 h-10 bg-gray-50 hover:bg-gray-100" onClick={onClose}>
          ...
          >
            ...
            <p className="text-sm text-gray-400 mt-2">
              (Solo aceptamos imágenes: JPEG, PNG)
            </p>
                ...
                {files.map((file) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {file.status === 'uploading' && <Progress value={file.progress} className="h-2 mt-2 bg-gray-100" />}
                    </div>
                    
                    {/* Botones de acción alineados a la derecha */}
                    <div className="flex items-center gap-1 shrink-0">
                      {file.status === 'completed' && <CheckCircle2 className="w-6 h-6 text-[#34d399] mr-1 hidden sm:block" />}
                      <Button variant="ghost" size="icon" className="rounded-full w-10 h-10 hover:bg-red-50 hover:text-red-500" onClick={(e) => {
                        e.stopPropagation();
                        onFileRemove(file.id);
                      }}>
                         {file.status === 'completed' ? <Trash2 className="w-5 h-5" /> : <X className="w-5 h-5" />}
                      </Button>
                    </div>
                  </motion.li>
                ))}
